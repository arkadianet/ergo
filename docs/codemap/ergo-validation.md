...
- `src/tx/mod.rs` (`validate_transaction`, `validate_transaction_parsed`, `CheckedTransaction`) — the 7-stage per-tx pipeline (deserialize → structural → canonical → resolve → heights → monetary → cost → script).
...
- `src/tx/mod.rs` — per-tx orchestration, `CheckedTransaction`, `TxValidationCtx`, canonical-encoding check, input/data-input resolution + match verification.
...
- `CheckedTransaction` (struct) — unforgeable validated tx (private fields); carries `tx_id` computed once + resolved inputs — `src/tx/mod.rs:52`
- `validate_transaction` / `validate_transaction_parsed` (fn) — full pipeline from bytes / from parsed-with-resolved-inputs (block path, supports `skip_scripts` checkpoint) — `src/tx/mod.rs:91` / `:176`
- `TxValidationCtx` (struct) — the per-tx borrow bundle (ctx, params, mutable cost, last_headers) — `src/tx/mod.rs:29`
...
- `PowCheckedHeader` (struct) — PoW-verified proof so the batch pipeline parallelizes PoW then finalizes sequentially — `src/header.rs:456`
- `validate_header` / `validate_header_after_pow` (fn) — only ways to mint a `CheckedHeader` from raw inputs — `src/header.rs:527` / `:495`
- `CheckedBlock` (struct) + `validate_full_block_parallel` (fn) — block proof object + production parallel validator; `validate_full_block` is the `#[cfg(test-helpers)]` sequential reference twin — `src/block.rs:814` / `:1445` / `:880`
- `BlockValidationContext` (struct) — parent header, UTXO view, params, voting_length, parent extension, soft-fork state, last headers, optional script-validation checkpoint — `src/block.rs:83`
- `SoftForkState` (struct) — soft-fork prohibited-vote window computation for rule 407 — `src/block.rs:36`
- `validate_interlinks` / `validate_extension_structural` / `validate_fork_vote` / `check_block_transactions_size` (fn) — the standalone block-level rule helpers (401/402, 400/404/405/406, 407, 306) — `src/block.rs:475` / `:541` / `:433` / `:404`
- `build_tx_layers` / `TxLayers` (fn/struct, crate-private) — topological layering of intra-block tx deps + intra-block double-spend rejection — `src/block.rs:725`
...
- `compute_next_params` (fn) — soft-fork + param recompute returning `(next_active, activated_update)` — `src/voting/recompute.rs:62`
...
- `ErgoValidationSettings` / `ErgoValidationSettingsUpdate` (struct) — rule-status set + soft-fork update with codec — `src/voting/validation_settings.rs:98` / `:152`
...
- `derive_activated_script_version` / `neutral_votes` (const fn) — `script_version = block_version - 1`; zeroed votes — `src/voting/mod.rs:63` / `:46`
