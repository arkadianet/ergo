...
**Approx LOC:** ~11,300 production (~19,300 incl. `evaluator/tests.rs`)

## Start here
- `reduce::verify_spending_proof_with_context` (`src/reduce.rs:114`) — the
  ...
  proof verify).
- `evaluator::dispatch::eval_op` (`src/evaluator/dispatch.rs:127`) — the
  `(opcode, payload)` jump table. The fastest map of *which opcodes exist* and
  which handler each routes to.
- `evaluator::types::Value` and `ReductionContext` (`src/evaluator/types.rs:307`,
  `:153`) — the runtime value enum and the transaction-scoped context every
  opcode reads. Understand these before any opcode handler.
- `verify::verify_sigma_proof` (`src/verify.rs:236`) — Fiat-Shamir proof
...
- `verify_spending_proof_with_context` (fn) — primary validation entry point;
  ...
  verify — `src/reduce.rs:114`
- `verify_spending_proof_with_context_and_cost` (fn) — same, threading an
  external `CostAccumulator` — `src/reduce.rs:132`
- `trivial_reduce` (fn) — P2PK/SigmaProp constant fast path — `src/reduce.rs:56`
...
- `AvlVerifier` (struct) — AVL+ proof boundary — `src/avl.rs:23`
- `blake2b256` (fn) + `SOUNDNESS_BYTES`/`GROUP_SIZE`/`SCHNORR_PROOF_SIZE`
  (consts) — `src/lib.rs:65`, `:53`
...
- **Trivial-reduction parity.** `trivial_reduce` only matches Scala's
  ...
  value) hard-reject (`src/reduce.rs:56-97`, `:150-170`).
- **Cost-model parity (consensus).** Per-opcode JitCost mirrors
  ...
  truncation (`src/reduce.rs:142-189`, `src/cost_table.rs`, `src/crypto_cost.rs`).
- **JitCost overflow / limit fidelity.** JitCost arithmetic overflow past the
  ...
  (`src/evaluator/types.rs:521-589`).
- **Fiat-Shamir / proof-tree byte parity.** Challenge size is 192 bits (24
  ...
  (`src/verify.rs:272-552`, `src/schnorr.rs:96-117`).
...
- **Soft-fork activation gate.** EIP-50 / Sigma-6.0 v6 MethodCalls require
  ...
  (`src/evaluator/types.rs:272-288`, `src/evaluator/opcodes/method_call.rs:48-50`).
- **Type-strict value equality.** `Value`'s `PartialEq` is type-strict like
  ...
  (`src/evaluator/types.rs:417-503`).
- **Depth bound.** Recursion is capped at `MAX_EVAL_DEPTH = 100`
  (`DepthLimitExceeded`) — `src/evaluator/dispatch.rs:111-114`,
  `src/evaluator/types.rs:592`.
