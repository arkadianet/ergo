...
- **Auth scope.** Protected operator routes include wallet access, admin controls, mining work and `/api/v1/diagnostics/activity` log reads. They fail closed when no `ApiSecurity` is configured. Public chain/status reads retain their existing access policy. `api_key` header (lowercase, underscore), Blake2b-256 → lowercase hex, constant-time compare, `403 {error,reason:"invalid.api-key",detail:null}` — Scala parity. The full-featured builder takes the gate as an explicit `Option`; each family mounts its protected routes with `route_layer`.
