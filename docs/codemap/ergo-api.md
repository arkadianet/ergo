...
- `src/compat/` — Scala API surface mounted at bare paths.
  ...
  - `compat/handlers.rs` — `/info`, `/blocks/*`, `/utxo/*`, `/peers/*`, `/transactions/unconfirmed/*` GET handlers, plus `/transactions/getFee` (recommendations floor at the configured minimum relay fee).
