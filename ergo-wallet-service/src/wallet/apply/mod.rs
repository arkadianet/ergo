//! Wallet apply / rollback hook. Called from the chain state
//! integration so wallet state advances atomically with chain state —
//! both writes share the same `WriteTransaction`, both commit or both abort.
//!
...
/// Minimal block-tx view the apply hook needs. The chain integration
/// constructs this from its full transaction representation.
...
pub struct BoundBlockTxs<'a> {
    outputs: Vec<Vec<BlockOutput<'a>>>,
    meta: Vec<([u8; 32], &'a [[u8; 32]])>,
}

impl<'a> BoundBlockTxs<'a> {
    pub fn as_block_txs(&self) -> Vec<BlockTx<'_>> {
        self.meta
            .iter()
            .zip(self.outputs.iter())
            .map(|((tx_id, inputs), outputs)| BlockTx {
                tx_id: *tx_id,
                inputs,
...
pub fn owned_to_block_txs(owned: &[crate::wallet::types::OwnedBlockTxData]) -> BoundBlockTxs<'_> {
    let outputs = owned
        .iter()
        .map(|data| {
            data.outputs
                .iter()
                .map(|output| BlockOutput {
    ...
    let meta = owned
        .iter()
        .map(|data| (data.tx_id, data.inputs.as_slice()))
        .collect();
...
/// directly. This keeps the persistence core independent of live wallet
/// state and chain-runtime types.
...
///
/// Production implementations abort the active rescan and write
/// `WALLET_SCAN_INVALIDATED = true` through the supplied transaction, so
/// rollback changes and invalidation commit atomically.
