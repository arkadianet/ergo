...
/// Rule-1001 classification, including definite non-SigmaProp sentinels.
/// This judgment is deliberately independent of exact substitution inference:
/// unknown tuple components or function ranges cannot make those roots SigmaProp.
...
mod tests {
    ...
    #[test]
    fn root_gate_projections_preserve_leniency_and_substitution_is_precise() {
        use crate::ergo_tree::{read_ergo_tree, substitution_type_of};
        ...
        for body in [
            ...
            "b2ad8301050500d90101057201040000", // Map then ByIndex
            "b2b58301050500d90101057f040000",   // Filter then ByIndex
        ] {
            let bytes = hex::decode(format!("00{body}")).unwrap();
            let tree = read_ergo_tree(&mut VlqReader::new(&bytes)).unwrap();
            assert_eq!(root(&tree.body), None, "{body}");
            assert_eq!(
                substitution_type_of(&tree.body),
                Some(SigmaType::SLong),
                "{body}"
            );

            // A lenient sized projection consumes the structural expression,
            // whereas a definite non-SigmaProp root consumes its declared region.
            let mut sized = vec![8, bytes.len() as u8];
            sized.extend_from_slice(&bytes[1..]);
            sized.push(0);
            let mut reader = VlqReader::new(&sized);
    ...
    #[test]
    fn root_gate_serialized_shapes_match_jvm_verdicts() {
        use crate::ergo_tree::{check_sigma_prop_root, read_ergo_tree};
        use ergo_primitives::reader::VlqReader;

        let cases: serde_json::Value = serde_json::from_str(include_str!(
            "../../../test-vectors/ergo-tree-root-gate/cases.json"
        ))
        .unwrap();
        let responses = include_str!("../../../test-vectors/ergo-tree-root-gate/responses.txt");
        let cases = cases.as_array().unwrap();
        assert_eq!(cases.len(), responses.lines().count());
        for (case, expected) in cases.iter().zip(responses.lines()) {
            ...
            if expected.starts_with("ACCEPT") {
                // JVM canonical bytes include the trailing byte in the declared
                // size region: a non-SigmaProp root wraps the entire region.
                assert_eq!(expected.strip_prefix("ACCEPT ").unwrap(), case["hex"]);
