...
pub struct AvlTreeData {
    ...
    /// Required length of every key, in bytes.
    pub key_length: u32,
...
fn read_avl_tree(r: &mut VlqReader) -> Result<AvlTreeData, ReadError> {
    ...
    let key_length = r.get_u32_exact()?;
    // Scala: r.getOption(r.getUInt().toInt) — reads 1-byte flag, then optional VLQ u32
    let has_value_length = r.get_u8()?;
    let value_length_opt = if has_value_length != 0 {
        Some(r.get_u32_exact()?)
    } else {
        None
    };
