...
use crate::store::{owned_to_block_txs, WalletApplyPayload, CHAIN_INDEX};
use crate::wallet::apply::{
    apply_block_to_scans, apply_block_to_scans_rescan, apply_block_to_wallet,
    apply_block_to_wallet_rescan, clear_scan_registry, clear_scan_tracking,
...
use crate::wallet::types::{
    Balance, BoxProvenance, BoxStatus, ScanTrackedBox, ScanTxRecord, TrackedPubkeyMeta, WalletBox,
    WalletTransaction,
};
...
pub trait WalletWrite {
    ...
    fn apply_rescan_block(
        ...
        scan_records: Option<&[crate::store::ScanMatchRecord]>,
    ) -> Result<(), WalletStoreError>;
...
impl WalletStore for Database {
    ...
    fn begin_write(&self) -> Result<Box<dyn WalletWrite>, WalletStoreError> {
        Ok(Box::new(RedbWalletWrite {
            owned: Some(crate::begin_write_qr(self)?),
            borrowed: None,
        }))
    }
}
...
impl WalletRead for RedbWalletRead {
    ...
    fn committed_tip(&self) -> Result<Option<(u32, [u8; 32])>, WalletStoreError> {
        crate::reader::committed_tip_in(&self.txn)
...
impl WalletWrite for RedbWalletWrite<'_> {
    ...
    fn prepare_rescan(
        ...
        let header_id = if height == 0 {
            ...
            let table = self.txn().open_table(crate::store::CHAIN_INDEX)?;
