...
impl DigestProofVerifier {
    ...
    pub fn apply_net_box_changes_resolving(
        ...
        // `proofs.verify` return value, the spent-input + data-input
        // boxes the validator needs.
        ...
        // ascending). A `BatchAVLVerifier` consumes its proof as a
        // stream, so each lookup — though read-only and digest-neutral —
        // must still be replayed here to keep the stream aligned for the
        // removes that follow. `to_lookup` carries the data-input box
        // ids in that exact order, duplicates included.
