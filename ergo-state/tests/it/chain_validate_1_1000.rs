//! State-backed transaction validation for blocks 1-1000.
//!
//! Validates every transaction through the full pipeline (structural, monetary,
//! script, cost) using real StateStore-backed UTXO lookups. Block 1 is applied
//! unchecked to bootstrap state; blocks 2-1000 are fully validated then applied.
...
fn load_header_info(path: &str) -> std::collections::HashMap<u32, ([u8; 32], [u8; 33], u64)> {
    ...
    for h in &headers {
        ...
        info.insert(height, (header_id, pk, header.timestamp));
    }
    info
}

/// Validate all transactions in blocks 2-1000 using real state store.
///
/// Block 1 bootstraps the state (applied unchecked). Every subsequent
/// transaction goes through the full validation pipeline: canonical encoding,
/// structural checks, monetary conservation, script evaluation with JIT cost,
/// and spending proof verification.
#[test]
fn chain_validate_blocks_2_1000_with_state_store() {
    ...
    let tx_data =
        std::fs::read_to_string("../test-vectors/mainnet/transactions_1_1000.json").unwrap();
    let vectors: Vec<TxVector> = serde_json::from_str(&tx_data).unwrap();
    let digests_data =
        std::fs::read_to_string("../test-vectors/mainnet/utxo_digests_1_1000.json").unwrap();
    let digests: Vec<DigestJson> = serde_json::from_str(&digests_data).unwrap();
    let header_info = load_header_info("../test-vectors/mainnet/headers_1_2000.json");
    ...
    let expected_digest_1 =
        ADDigest::from_bytes(hex::decode(&d1.state_root).unwrap().try_into().unwrap());
    let (header_id_1, _, _) = *header_info.get(&1).unwrap();
    store
        .apply_block_unchecked_for_test(1, &header_id_1, &expected_digest_1, &[tx1])
        .unwrap();

    // Validate and apply blocks 2-1000
    let mut validated = 0;
    let mut total_cost: u64 = 0;
    for v in vectors.iter().skip(1) {
        let tx_bytes = hex::decode(&v.bytes).unwrap();
        let (header_id, miner_pubkey, timestamp) = *header_info.get(&v.height).unwrap();
    ...
    assert_eq!(
        validated, 999,
        "should validate 999 transactions (blocks 2-1000)"
    );
    assert_eq!(store.height(), 1000);
    eprintln!("Validated {validated} transactions, total cost: {total_cost}");
}
