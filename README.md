...
| Mode 4 — Pruned + UTXO bootstrap | yes | yes for the composed lifecycle: a real UTXO-snapshot install and a NiPoPoW proof compose max-style on the prune sentinel and reboot cleanly (tested both orders, `ergo-node/tests/it/mode4_acceptance.rs`). Live multi-peer soak still outstanding |
