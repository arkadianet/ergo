//! Snapshot of the native `/api/v1/*` OpenAPI YAML.
//!
//! The derive in `server.rs` generates the spec from the handler
//! annotations and DTO `ToSchema` derives. This test pins the generated
//! YAML against a checked-in golden file so a handler-signature or DTO
//! change that isn't mirrored in the `#[utoipa::path]` / `#[derive]`
//! annotations fails loudly here instead of silently shipping a wrong
//! spec.
//!
//! It proves what the derive *emitted* — not what the router *mounted*.
//! Route wiring is covered separately by `openapi_native_runtime_mount`.
//!
...
use ergo_api::server::native_openapi_yaml;
...
#[test]
fn openapi_native_matches_snapshot() {
    ...
    if actual != expected {
        panic!(
            "native OpenAPI spec drifted from the checked-in golden file.\n{}\n\
