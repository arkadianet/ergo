...
Run from the repository root after committing the extractor:
...
explicit capture timestamp so re-runs can retain it. The script checks that the
provenance commit contains its exact source. No node or blockchain context is
...
- `constants`: reflection discovers all `OperationCostInfo` getters on
  ...
  storage cost, evaluator block size, and JitCost representation bounds/scale.
...
The selected/executed counts count opcode records, version-grouped method
