...
impl PeerManager {
    ...
    pub fn sticky_peers_needing_dial(&self, now: Instant) -> Vec<SocketAddr> {
        ...
        for addr in &self.priority_peers {
            ...
            // Dormant: persistently-failing sticky peer. Redialing a
            // peer that does not accept sessions at short intervals
            // indefinitely wastes dial budget and keeps connection
            // pressure on that host. Success zeroes
