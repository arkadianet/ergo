...
pub(super) fn gate(command: WalletCommand, store: &dyn WalletStore) -> Option<WalletCommand> {
        ...
        ($($variant:ident),+ $(,)?) => {
            match command {
                $(command @ WalletCommand::$variant { .. } => {
                    match require_valid_scan(store) {
                        Ok(()) => Some(command),
                        Err(error) => {
...
mod tests {
    ...
    fn scan_gate_unreadable_flag_refuses_command() {
        ...
        let store = ergo_state::wallet::RedbWalletStore::new(std::sync::Arc::new(db));
        assert!(gate(WalletCommand::Balances { reply }, &store).is_none());
        assert!(matches!(
