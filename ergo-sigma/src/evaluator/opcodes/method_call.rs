...
pub(in crate::evaluator) fn eval_method_call(
    ...
    match (type_id, method_id) {
        ...
        (100, 13) => {
            ...
            // isUpdateAllowed check: FixedCost(15)
            cx.cost.add(JitCost::from_jit(15))?;
            if !avl.update_allowed {
                return Ok(Value::Opt(None));
            }
            // Extract entries: Coll[(Coll[Byte], Coll[Byte])]. Outer
            // is `CollGeneric` (boxed-element coll); each inner is a
            // real 2-tuple `Value::Tuple` pair.
            let entries_val = cx.eval_expr(&args[0])?;
            let entries: Vec<(Vec<u8>, Vec<u8>)> = match entries_val {
                Value::CollGeneric(items, _) => items
                    .into_iter()
                    .map(|item| match item {
                        Value::Tuple(pair) if pair.len() == 2 => {
                            ...
                            let k = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for update key",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                            let v = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for update value",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                        ...
                        other => Err(EvalError::TypeError {
                            expected: "(Coll[Byte], Coll[Byte]) in entries",
                            got: format!("{other:?}"),
                        }),
                    })
                    .collect::<Result<_, _>>()?,
                other => {
                    return Err(EvalError::TypeError {
                        expected: "Coll[(Coll[Byte], Coll[Byte])] for update entries",
                        got: format!("{other:?}"),
                    })
                }
            };
            ...
            // Cost: CreateAvlVerifier(proof.len)
            let create_cost = CostKind::PerItem {
                base: JitCost::from_jit(110),
                per_chunk: JitCost::from_jit(20),
                chunk_size: 64,
            ...
            cx.cost.add(create_cost.compute(proof.len() as u32)?)?;
            let mut bv = make_avl_verifier(&avl, &proof)?;
            let tree_height = (avl.digest.tree_height_byte() as u32).max(1);
            let update_cost = CostKind::PerItem {
                base: JitCost::from_jit(120),
                per_chunk: JitCost::from_jit(20),
                chunk_size: 1,
            };
            // Per-entry: charge UpdateAvlTree, perform operation, break on failure
            for (key, value) in entries {
                cx.cost.add(update_cost.compute(tree_height)?)?;
                if bv.update(&key, &value).is_err() {
                    break;
                }
            }
            // updateDigest: FixedCost(40)
            match bv.digest() {
                Some(d) => {
                    cx.cost.add(JitCost::from_jit(40))?;
                    if d.len() != 33 {
                        return Ok(Value::Opt(None));
                    }
        ...
        (100, 12) => {
            ...
            let entries_val = cx.eval_expr(&args[0])?;
            let entries: Vec<(Vec<u8>, Vec<u8>)> = match entries_val {
                // Outer `CollGeneric` carries Coll[(Coll[Byte], Coll[Byte])];
                // each inner pair is a real `Value::Tuple`.
                Value::CollGeneric(items, _) => items
                    .into_iter()
                    .map(|item| match item {
                        Value::Tuple(pair) if pair.len() == 2 => {
                            ...
                            let k = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for insert key",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                            let v = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for insert value",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                ...
                other => {
                    return Err(EvalError::TypeError {
                        expected: "Coll[(Coll[Byte], Coll[Byte])] for insert entries",
                        got: format!("{other:?}"),
                    })
                }
            };
            ...
            let mut bv = make_avl_verifier(&avl, &proof)?;
            let tree_height = (avl.digest.tree_height_byte() as u32).max(1);
            let insert_cost = CostKind::PerItem {
                base: JitCost::from_jit(120),
                per_chunk: JitCost::from_jit(20),
                chunk_size: 1,
            };
            for (key, value) in entries {
                cx.cost.add(insert_cost.compute(tree_height)?)?;
                if bv.insert(&key, &value).is_err() {
                    break;
                }
            }
            match bv.digest() {
        ...
        (100, 16) => {
            ...
            cx.cost.add(JitCost::from_jit(15))?;
            if !avl.insert_allowed || !avl.update_allowed {
                return Ok(Value::Opt(None));
            }
            let entries_val = cx.eval_expr(&args[0])?;
            let entries: Vec<(Vec<u8>, Vec<u8>)> = match entries_val {
                    ...
                    .map(|item| match item {
                        Value::Tuple(pair) if pair.len() == 2 => {
                            ...
                            let k = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for insertOrUpdate key",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                            let v = match it.next().unwrap() {
                                ...
                                other => {
                                    return Err(EvalError::TypeError {
                                        expected: "Coll[Byte] for insertOrUpdate value",
                                        got: format!("{other:?}"),
                                    })
                                }
                            };
                ...
                other => {
                    return Err(EvalError::TypeError {
                        expected: "Coll[(Coll[Byte], Coll[Byte])] for insertOrUpdate entries",
                        got: format!("{other:?}"),
                    })
                }
            };
            ...
            let mut bv = make_avl_verifier(&avl, &proof)?;
            let tree_height = (avl.digest.tree_height_byte() as u32).max(1);
            let op_cost = CostKind::PerItem {
                base: JitCost::from_jit(120),
                per_chunk: JitCost::from_jit(20),
                chunk_size: 1,
            };
            for (key, value) in entries {
                cx.cost.add(op_cost.compute(tree_height)?)?;
                if bv.insert_or_update(&key, &value).is_err() {
                    break;
                }
            }
            match bv.digest() {
        ...
        (100, 14) => {
            ...
            // keys: Coll[Coll[Byte]] â€” outer is the boxed-element coll
            // carrier, each inner key is a typed `CollBytes`.
            let keys_val = cx.eval_expr(&args[0])?;
            let keys: Vec<Vec<u8>> = match keys_val {
                Value::CollGeneric(items, _) => items
                    .into_iter()
                    .map(|item| match item {
                        Value::CollBytes(k) => Ok(k),
                        other => Err(EvalError::TypeError {
                            expected: "Coll[Byte] in remove keys",
                            got: format!("{other:?}"),
                        }),
                    })
                    .collect::<Result<_, _>>()?,
                other => {
                    return Err(EvalError::TypeError {
                        expected: "Coll[Coll[Byte]] for remove keys",
                        got: format!("{other:?}"),
                    })
                }
            };
            ...
            for key in &keys {
                cx.cost.add(remove_cost.compute(tree_height)?)?;
                if bv.remove(key).is_err() {
                    break;
                }
            }
            match bv.digest() {
