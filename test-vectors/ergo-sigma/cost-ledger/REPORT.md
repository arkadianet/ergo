...
|---|---|
| Per-input crypto truncation | Truncate each crypto estimate to block units before accumulation; prevents remainder carry and reject-valid exact-limit failures (`INTERP-crypto-trunc`, `ROUND-crypto-per-input`). |
| Storage-rent unit | Charge StorageContractCost as 50 block units (`TX-storage-rent`). |
| Empty SigmaAnd / SigmaOr | Reject empty children as JVM CAND/COR normalization does (`OP-0xEA`, `OP-0xEB`). |
| BitOp charge before rejection | Inherited evaluator rejects without charging its declared Fixed(1); independent evaluator failure observations pin this (`ORDER-bitop-charge-then-reject`). |
| Collection equality counting | Charge the elements actually compared; unknown-length charges remain deferred (`EVAL-eq-coll-descriptor`). |
| Unit equality | Charge zero for Unit equality (`EVAL-eq-mismatch-and-unit-E032`); element-type mismatch remains unproven in that row's note. |
| String equality / rejection | Reproduce shallow SString rejection and descriptor-carrier guards in JVM order (`EVAL-sstring-rejected`). |
| Option charge order | Evaluate receiver/arguments before method tariffs; Some paths include AddToEnvironment, including Option.filter. |
| Method failure-path order | Align receiver, argument, fixed-method, AVL, serialization and branch charge order with JVM per-limit observations (`ORDER-*` CLOSED rows). |
| startsWith / endsWith length | Charge using receiver length, not the compared prefix/suffix length (`METHOD-coll-startsEndsWith`). |
| Embedded-script bytes | Charge deserializeMeasured bytes for embedded scripts (`INTERP-embedded-script-deser`). |
| Threshold clamp | Use n-k without a minimum-one clamp: 3-of-3 has 1197 crypto BC and total 1219 (`INTERP-crypto-threshold`). |
| Deserialize substitution typing | Reject incompatible registers and untyped substitutions; preserve the independent rule-1001 root gate (commits `23130988`, `5b65b4f9`, `1d8ddde3`). |
| Historical voted parameters | Replay/extractor uses historical epoch parameters rather than present defaults (`TX-l4-range-voted-params`). |
| Empty token-map typing | Preserve empty map element types to accept valid mainnet spends (`TX-l4-v6-activation-reject-valid`, `3e249d88`). |
| Emission-box discovery | Track lineage, retain unspent emission boxes, handle exhaustion and restart recovery (`BLOCK-L6-emission-box-discovery`). |
| Mining safety gap | Use Scala tiers rather than the fixed subtraction (`BLOCK-L6-mining-safety-gap`). |

Program PRs: [#337](https://github.com/arkadianet/ergo/pull/337), [#338](https://github.com/arkadianet/ergo/pull/338), [#339](https://github.com/arkadianet/ergo/pull/339), [#340](https://github.com/arkadianet/ergo/pull/340), [#341](https://github.com/arkadianet/ergo/pull/341), [#342](https://github.com/arkadianet/ergo/pull/342), [#343](https://github.com/arkadianet/ergo/pull/343), [#344](https://github.com/arkadianet/ergo/pull/344). PR-E is the controller-designated carrier for work after `3e249d88`; no new PR or push is performed by this task. No CLOSED row currently contains a “PR pending” note.
