...
## Instrumentation and voted transitions

The `g-`, `h-`, `i-`, and `j-` families are generated by the block oracle's
`instrumentation-self-test` command; see its README for the ten cases and epoch
bootstrap. Optional fields extend schema version 1:

| Field | Meaning |
|---|---|
| `expected.transaction_entries` | Production loop-entry index and accumulated cost before that transaction; absence of an entry proves the loop never validated or charged it. |
| `digest_expected` | Production DigestState application result, including its delegated cost before conversion to Unit. Same parent, context, target and proofs as the UTXO observation. |
| `epoch_vote` | Signed parameter ID proposed at height 256 and voted through 383; builds 383 parent blocks. |
| `transition.previous_parameters` | Parent's parameter table before the target epoch. |
| `transition.updated_parameters` | JVM table after appending the target extension at height 384. |
| `transition.epoch_votes` | Actual JVM parent-context tally. Rust recounts the serialized parent headers. |
| `transition.stale_verdict`, `transition.stale_cost` | Production transaction execution with target headers but stale parent parameters. |

The Rust consumer reconstructs every parent root, recomputes voted parameters,
compares the target extension to the JVM table, and validates using the updated
parameters. Digest cases resolve boxes from authenticated ADProofs before running
both Rust validators. Rejected execution never substitutes a guessed partial sum.
