...
`gate-soft-fork.json.gz` is consumed by both the per-script verifier and the
production transaction-script fixture runner.
...
`ProtocolParams::from_active` carries cumulative activated statuses into
`ReductionContext`; pending updates do not affect script verification. The
activated-version gate still runs before reduction. `validation-settings.json.gz`
contains 103 JVM cases spanning default, disabled, replaced and changed statuses,
A6 rule identities, unrelated failures and cost limits. These fixtures close
VERSION-tree-version-gate, VERSION-G019 and VERSION-G020 together with mainnet
replay using real epoch settings (101,187 transactions, zero divergences).
Disabled status alone does not recognize a soft fork for the exercised rules.
...
scripts/gen-cost-fixture.sh test-vectors/ergo-sigma/cost-ledger/fixtures/version/gate-soft-fork.json.gz
```

Refresh the settings matrix from its stored requests:

```sh
scripts/gen-cost-fixture.sh test-vectors/ergo-sigma/cost-ledger/fixtures/version/validation-settings.json.gz
```
