...
| `block_545684_future_version_tree.json` (header + blockTransactions only) | `api_bridge::tests::block_545684_*` (ergo-node) | **future-version ErgoTree on chain**: tx[1] output[0] is `cd07021a8e6f59fd4a` — size bit set, header version 5 — accepted by Scala below `VersionContext.JitActivationVersion` and kept as an `UnparsedErgoTree`. Backs the stored-section re-read fix for issue #326; the section bytes are bound to the fixture header's `transactionsRoot` by the test itself. |
