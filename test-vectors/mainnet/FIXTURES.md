...
| `block_545684_future_version_tree.json` (header + blockTransactions only) | `api_bridge::tests::block_545684_*` (ergo-node) | **future-version ErgoTree on chain**: tx[1] output[0] is `cd07021a8e6f59fd4a` — size bit set, header version 5 — accepted by Scala below `VersionContext.JitActivationVersion` (the `ergoTreeVersion <= activatedVersion` require is inert) and kept as an `UnparsedErgoTree`. Backs the stored-section re-read fix for issue #326 and the activated-version keying of `check_tree_version_supported` (#327); the section bytes are bound to the fixture header's `transactionsRoot` by the test itself. JVM verdicts for the same bytes at activated 1/2/3 are pinned in `test-vectors/scala/tree_version_activated_oracle.json`. |
| `headers_json/scala_headers_v1_mainnet.json` (10 header bodies, verbatim `GET /blocks/{id}/header`, h1-9 + h15) | `headers_json_scala_oracle.rs` (ergo-rest-json) v1 JSON→bytes oracle | **Autolykos v1 `d`**: Scala writes the PoW distance with `BigIntegers.asUnsignedByteArray` (unsigned, minimal-length) and circe renders it as a bare JSON number. Seven of the ten headers (3, 4, 5, 7, 8, 9, 15) have a high-bit `d` — h3 is the first on the chain — so the fixture discriminates an unsigned from a signed decode; the other three (1, 2, 6) are the low-byte control. Ids cross-checked against the independently captured wire bytes in `headers_1_10.json`. |
