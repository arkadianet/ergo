...
pub const EMISSION_NFT_ID: [u8; 32] = [
    0x20, 0xfa, 0x2b, 0xf2, 0x39, 0x62, 0xcd, 0xf5,
    0x1b, 0x07, 0x72, 0x2d, 0x62, 0x37, 0xc0, 0xc7,
    0xb8, 0xa4, 0x4f, 0x78, 0x85, 0x6c, 0x0f, 0x7e,
    0xc3, 0x08, 0xdc, 0x1e, 0xf1, 0xa9, 0x2a, 0x51,
];

/// Re-emission token ID (mainnet).
pub const REEMISSION_TOKEN_ID: [u8; 32] = [
    0xd9, 0xa2, 0xcc, 0x8a, 0x09, 0xab, 0xfa, 0xed,
    0x87, 0xaf, 0xac, 0xfb, 0xb7, 0xda, 0xee, 0x79,
    0xa6, 0xb2, 0x6f, 0x10, 0xc6, 0x61, 0x3f, 0xc1,
    0x3d, 0x3f, 0x39, 0x53, 0xe5, 0x52, 0x1d, 0x1a,
];
...
pub fn verify_reemission_spending(
    ...
    let emission_input = input_boxes.iter().find(|b| {
        has_token(b, &EMISSION_NFT_ID) && b.candidate.value > MIN_EMISSION_BOX_VALUE
    });
...
mod tests {
    ...
    fn test_reemission_tokens_not_burned() {
        ...
        assert!(matches!(result, Err(ReemissionError::ReemissionTokensNotBurned)));
    }
    ...
    fn test_emission_nft_not_preserved() {
        ...
        let result = verify_reemission_spending(&tx, &[input_box], 800_000);
        assert!(result.is_err());
        assert!(matches!(result, Err(ReemissionError::EmissionNftNotPreserved)));
    }
