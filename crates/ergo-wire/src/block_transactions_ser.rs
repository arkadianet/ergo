...
/// [VLQ UInt: tx_count]
/// for each tx:
///   [VLQ UInt: tx_byte_length]
///   [tx_byte_length bytes: tx_bytes]
/// ```
pub fn serialize_block_transactions(bt: &BlockTransactions) -> Vec<u8> {
    ...
    // each transaction: length-prefixed opaque bytes
    for tx in &bt.tx_bytes {
        put_uint(&mut buf, tx.len() as u32);
...
pub fn parse_block_transactions(data: &[u8]) -> Result<BlockTransactions, CodecError> {
    ...
    for _ in 0..tx_count {
        let tx_len = get_uint(reader)? as usize;
        let tx_data = read_bytes(reader, tx_len)?;
        tx_bytes.push(tx_data);
    }
...
mod tests {
    ...
    fn block_transactions_roundtrip_v2() {
        let bt = BlockTransactions {
            ...
            block_version: 2,
            tx_bytes: vec![vec![0xDE, 0xAD, 0xBE, 0xEF]],
        };
        let bytes = serialize_block_transactions(&bt);
        let parsed = parse_block_transactions(&bytes).unwrap();
        assert_eq!(parsed, bt);
    ...
    fn block_transactions_parse_truncated_returns_error() {
        let bt = BlockTransactions {
            ...
            block_version: 2,
            tx_bytes: vec![vec![0x01, 0x02, 0x03, 0x04, 0x05]],
        };
        let bytes = serialize_block_transactions(&bt);
